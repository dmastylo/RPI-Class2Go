...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """Change ContentGroup.display_style to list style everywhere.

        The original plan for ContentGroup.display_style was to default to
        button-style display, to make the UI more compact, and then to only make
        certain specific items display their parent/child relationship in the
        indented-list style.
        
        After living with both in testing for a bit, nobody really likes button-style
        display, compactness isn't so very important, and making list display the
        default seems like obviously the right thing to do.
        """
        for contentgroup in orm.ContentGroup.objects.all():
            contentgroup.display_style = 'list'
            contentgroup.save()

    def backwards(self, orm):
        print "This migration isn't reversible, but trying is no error."
